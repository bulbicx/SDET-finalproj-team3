...
public class PublicUserControllerIntegrationTest {
	...
	void testCreateUser() throws Exception {
		// Create User object
		PublicUser user = new PublicUserBuilder().name("Micheal90").username("Michael").password("password123")
				.playlists(new ArrayList<>()).build();

		// Convert it to a JSON String
		this.mapper.setAnnotationIntrospector(new IgnoreJacksonWriteOnlyAccess());
	...
	void testUpdateUser() throws Exception {
		...
		// Convert it to a JSON String
		this.mapper.setAnnotationIntrospector(new IgnoreJacksonWriteOnlyAccess());
		String updatedUserAsJSON = this.mapper.writeValueAsString(user);

		// Build mock request
		RequestBuilder mockRequest = put("/users/public/update/1").contentType(MediaType.APPLICATION_JSON)
				.content(updatedUserAsJSON);

		PublicUser expectedUser = new PublicUserBuilder().name("Micheal90").username("Michaels")
				.playlists(new ArrayList<>()).build();
		// Get status created
		String expectedUserAsJSON = this.mapper.writeValueAsString(expectedUser);
		ResultMatcher matchBody = content().json(expectedUserAsJSON);
		// Get status created
